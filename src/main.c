...
static uint32_t client_ip_addr = IPv4(0, 0, 0, 0);

/* the server side */
static struct ether_addr server_ether_addr =
    {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};
static uint32_t server_ip_addr = IPv4(0, 0, 0, 0);
...
pingpong_parse_args(int argc, char **argv)
{
    ...
    while ((opt = getopt_long(argc, argv, short_options, longopts, &arg_index)) != EOF)
    {
        ...
        case OPT_CLIENT_IP:
            ...
            client_ip_addr = IPv4(ip_bytes[0], ip_bytes[1], ip_bytes[2], ip_bytes[3]);
            break;
        ...
        case OPT_SERVER_IP:
            ...
            server_ip_addr = IPv4(ip_bytes[0], ip_bytes[1], ip_bytes[2], ip_bytes[3]);
            break;
        ...
        while (!received_pong && !force_quit)
        {
            ...
            if (nb_rx)
            {
                ...
                for (i = 0; i < nb_rx; i++)
                {
                    ...
                    {
                        vlan_hdr = (struct rte_vlan_hdr *)((char *)eth_hdr + sizeof(struct ether_hdr));
